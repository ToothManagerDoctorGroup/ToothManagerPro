...
const static CGFloat kCustomIOS7DismissButtonHeight                = 22.0;
...
@synthesize useMotionEffects,useDismissButton;
...
- (UIView *)createContainerView
{
    ...
    //There is a line between the button
    UIView *lineViewMid = [[UIView alloc] initWithFrame:CGRectMake((dialogContainer.bounds.size.width - buttonSpacerHeight) / 2, CGRectGetMaxY(lineView.frame), buttonSpacerHeight, dialogContainer.bounds.size.height - CGRectGetMaxY(lineView.frame))];
    lineViewMid.backgroundColor = [UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:1.0f];
    [dialogContainer addSubview:lineViewMid];
    ...
    [dismiss setImage:[UIImage imageNamed:@"alert_close"] forState:UIControlStateNormal];
    dismiss.hidden = !useDismissButton;
    [dismiss addTarget:self action:@selector(close) forControlEvents:UIControlEventTouchUpInside];
    dismiss.frame = CGRectMake(dialogContainer.bounds.size.width - kCustomIOS7DismissButtonHeight - 5, 5, kCustomIOS7DismissButtonHeight, kCustomIOS7DismissButtonHeight);
    [dialogContainer addSubview:dismiss];
