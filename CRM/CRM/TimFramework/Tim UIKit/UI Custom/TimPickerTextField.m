...
@interface TimPickerTextField () <UIPickerViewDelegate,UIPickerViewDataSource>
{
...
@implementation TimPickerTextField 
...
- (void)onFinishButtonAction:(UIBarButtonItem *)sender {
    ...
    } else if (self.mode == TextFieldInputModeCustomPicker) {
    ...
    } else if (self.mode == TextFieldInputModeKeyBoard) {
        if (self.actiondelegate) {
            [self.actiondelegate pickerViewFinish:_inputPickerView];
        }
    }
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    if (self.dataSource) {
        ...
        if (self.mode == TextFieldInputModePicker) {
            self.text = [self.pickerDataSource objectAtIndex:row];
        }
        return rowView;
    }
    return view;
