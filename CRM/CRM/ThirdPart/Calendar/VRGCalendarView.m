...
@implementation VRGCalendarView{
    ...
    NSMutableArray *startDateArray1;
...
{       
    ...
    if (touchPoint.y > kVRGCalendarViewTopBarHeight) {
        ...
        /*
        int targetColumn = blockNr%7;
        int targetX = targetColumn * (kVRGCalendarViewDayWidth+2);
        int targetRow = blockNr/7;
        int targetY = kVRGCalendarViewTopBarHeight + targetRow * (kVRGCalendarViewDayHeight+2);
        
        UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(targetX, targetY, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight)];
        label.text = @"我们";
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        /*
        UIImageView *imaView = [[UIImageView alloc]initWithFrame:CGRectMake(targetX+kVRGCalendarViewDayWidth-9, targetY+kVRGCalendarViewDayHeight-9, 10, 10)];
        [imaView setBackgroundColor:[UIColor redColor]];
        imaView.hidden = YES;
        imaView.tag = i;
        ...
        self.selectedDate = [gregorian dateFromComponents:comps];
         ...
         NSString *startDateString = [dateFormatter stringFromDate:self.selectedDate];
        
        [startDateArray addObject:startDateString];
        
*/
        /*
         NSArray *remindArray = [[NSArray alloc]init];
         remindArray = [[LocalNotificationCenter shareInstance] localNotificationListWithString:startDateString];
        if(remindArray.count > 0){
            UIImageView *ima = (UIImageView *)[self viewWithTag:i];
            ima.hidden = NO;
        }
    }
        NSMutableArray *array1 = [[NSMutableArray alloc]initWithCapacity:0];
        for(NSInteger i =0;i<startDateArray.count;i++){
            if([startDateString isEqualToString:startDateArray[i]]){
                
            }else{
                [array1 addObject:startDateString];
            }
        }
        startDateArray1 = array1;
    }
    */
    }
    ...
    /*
    for(NSInteger i =0;i<[self numRows]*7;i++){
        int targetRow = i/7;
        int targetY = kVRGCalendarViewTopBarHeight + targetRow * (kVRGCalendarViewDayHeight+2);
        UIImageView *imaView = [[UIImageView alloc]initWithFrame:CGRectMake(i%7 * (kVRGCalendarViewDayWidth+2)+kVRGCalendarViewDayWidth-9, targetY+kVRGCalendarViewDayHeight-9, 10, 10)];
        [imaView setBackgroundColor:[UIColor redColor]];
        imaView.hidden = YES;
        imaView.tag = i;
        [self addSubview:imaView];
    ...
    NSString *string = [startDateArray componentsJoinedByString:@"|"];
    NSArray *remindArray = [[NSArray alloc]init];
    remindArray = [[LocalNotificationCenter shareInstance] localNotificationListWithString1:string];
    for(NSInteger i = 0;i<remindArray.count;i++){
        NSArray *dataArray = remindArray[i];
...
-(void)updateRed{
    ...
    [startDateArray1 removeAllObjects];
    for(NSInteger i =0;i<[self numRows]*7;i++){
        ...
        NSDateComponents *comps = [gregorian components:NSYearCalendarUnit | NSMonthCalendarUnit |  NSDayCalendarUnit fromDate:self.currentMonth];
        int firstWeekDay = [self.currentMonth firstWeekDayInMonth]-1; //-1 because weekdays begin at 1, not 0
        int date = i-firstWeekDay+1;
        [comps setDay:date];
        NSDate *thisDate = [gregorian dateFromComponents:comps];
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
        [dateFormatter setDateFormat:@"yyyy-MM-dd"];
        NSString *startDateString = [dateFormatter stringFromDate:thisDate];
        [startDateArray addObject:startDateString];
        NSMutableArray *array1 = [[NSMutableArray alloc]initWithCapacity:0];
        for(NSInteger i =0;i<startDateArray.count;i++){
            if([startDateString isEqualToString:startDateArray[i]]){
...
-(id)init {
    ...
    if (self) {
        ...
        startDateArray1 = [[NSMutableArray alloc]init];
        isAnimating=NO;
        self.labelCurrentMonth = [[UILabel alloc] initWithFrame:CGRectMake(50, 2, kVRGCalendarViewWidth-100, 40)];
