...
- (id)initWithView:(UIView*)view
{
    ...
    _endRect = self.contentView.frame;
    return self;   
}
...
- (void) showPopoverWithRect:(CGRect)senderRect withParentView:(UIView*)parentView withFrontViewTag:(NSInteger)tag
{
    _frontViewTag = tag;
    if (senderRect.origin.y<240) {
        _startRect = CGRectMake(_endRect.origin.x,
                                _endRect.origin.y-self.contentView.frame.size.height,
    ...
    self.contentView.frame = _startRect;
    
    [self.view addSubview:self.contentView];
    
    [parentView insertSubview:self.view belowSubview:[parentView viewWithTag:_frontViewTag]];
    ...
    [UIView animateWithDuration:0.2
                     ...
                     animations:^{
                         if (senderRect.origin.y<240) {
                             self.contentView.frame = CGRectMake(_startRect.origin.x,
                                                                 _startRect.origin.y+self.contentView.frame.size.height,
                                                                 self.contentView.frame.size.width,
                                                                 self.contentView.frame.size.height);
                         }else{
                             self.contentView.frame = CGRectMake(_startRect.origin.x,
                     ...
                     completion:^(BOOL finished) {
                         ...
                         [self.view insertSubview:touchView belowSubview:self.contentView];
                     }
     ];
}
...
- (void) dismissPopoverAnimatd:(BOOL)animated
{
    if (self.view) {
        if(animated) {
            ...
            [parent bringSubviewToFront:[parent viewWithTag:_frontViewTag]];
            
            [UIView animateWithDuration:0.2
                                  delay:0.0
                                options:UIViewAnimationOptionAllowAnimatedContent
                             animations:^{
                                 //spopoverView.alpha = 0;
                                 self.contentView.frame = _startRect;
                             }
                             completion:^(BOOL finished) {
