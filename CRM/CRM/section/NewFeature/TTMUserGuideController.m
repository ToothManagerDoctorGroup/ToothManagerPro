...
- (void)viewDidLoad {
    ...
    self.view.backgroundColor = MyColor(240.0, 240.0, 240.0);
...
- (void)scrollToNext {
    ...
    if (lastX == ((_images.count - 1) * kScreenWidth)) {
        [self.timer invalidate];
        self.timer = nil;
        return;
    }
    [self.scrollView setContentOffset:CGPointMake(lastX + kScreenWidth, 0) animated:YES];
}
