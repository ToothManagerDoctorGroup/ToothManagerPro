...
- (void)setTagsFrame:(XLTagFrame *)tagsFrame{
    ...
    for (NSInteger i=0; i<tagsFrame.tagsArray.count; i++) {
        ...
        tagsBtn.tag = i;
        tagsBtn.backgroundColor = _clickBackgroundColor;
    ...
    for (id obj in self.subviews) {
        if ([obj isKindOfClass:[UIButton class]]) {
            ...
            if (btn.tag == index){
                ...
                if (self.delegate && [self.delegate respondsToSelector:@selector(tagView:tagArray:)]) {
                    [_delegate tagView:self tagArray:@[[NSString stringWithFormat:@"%ld",(long)index]]];
                }
            }else{
    ...
    for (id obj in self.subviews) {
        if ([obj isKindOfClass:[UIButton class]]) {
            ...
            if (btn.tag == [index integerValue]){
                ...
                if ([selectedTagBtnList containsObject:index]) {
                    
                    btn.backgroundColor = [UIColor whiteColor];
                    [btn setTitleColor:TextColor forState:UIControlStateNormal];
                    [self makeCorner:_borderSize view:btn color:UIColorRGBA(221, 221, 221, 1)];
                    [selectedTagBtnList removeObject:index];
                    ...
                    [btn setTitleColor:_clickTitleColor forState:UIControlStateNormal];
                    [self makeCorner:_clickborderSize view:btn color:_clickTitleColor];
                    [selectedTagBtnList addObject:index];
                    
                }
                
                if (self.delegate && [self.delegate respondsToSelector:@selector(tagView:tagArray:)]) {
                    [_delegate tagView:self tagArray:selectedTagBtnList];
                }
            }
        }
...
-(void)TagsBtn:(UIButton *)sender{
    ...
    if (self.selectType == XLTagViewSelectTypeSingle) {
        //单选
