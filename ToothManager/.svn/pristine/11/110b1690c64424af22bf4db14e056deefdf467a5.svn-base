...
@interface TTMPopoverView ()<UITableViewDataSource, UITableViewDelegate>
...
-(CGRect)getViewFrame
{
    ...
    CGFloat maxH = ROW_HEIGHT * 7;
    CGFloat height = [self.titleArray count] * ROW_HEIGHT + SPACE + kArrowHeight;
    if (height > maxH) {
    ...
    for (NSString *title in self.titleArray) {
        CGFloat width =  [title boundingRectWithSize:CGSizeMake(300, 100)
                                             options:NSStringDrawingTruncatesLastVisibleLine
                                          attributes:@{@"font": TITLE_FONT}
    ...
    //右间隔最小5x
    CGFloat maxRight = ScreenWidth - 5;
    if ((frame.origin.x + frame.size.width) > maxRight) {
