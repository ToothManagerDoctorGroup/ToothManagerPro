...
@interface MCPopoverView ()<UITableViewDataSource, UITableViewDelegate>
...
- (CGRect)viewFrame
{
    ...
    if (height > (ScreenHeight - NavigationHeight - TabbarHeight)) {
        frame.size.height = (ScreenHeight - 200);
    } else {
        frame.size.height = [self.titleArray count] * ROW_HEIGHT + SPACE + kArrowHeight;
    }
    
    for (NSString *title in self.titleArray) {
        NSDictionary *attriDic = @{NSFontAttributeName : TITLE_FONT};
