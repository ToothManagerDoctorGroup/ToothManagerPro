...
- (id)initWithStyle:(HZAreaPickerStyle)pickerStyle delegate:(id<HZAreaPickerDelegate>)delegate
{
    if (self = [super init]) {
        ...
        [self addTarget:self action:@selector(cancelPicker) forControlEvents:UIControlEventTouchUpInside];
        self.frame = CGRectMake(0, 0, ScreenWidth, ScreenHeight);
        
        // pickerView
        self.locatePicker = [[UIPickerView alloc] initWithFrame:CGRectMake(0, ScreenHeight, ScreenWidth, kViewH)];
...
- (void)showInView:(UIView *)view
{
    ...
    [UIView animateWithDuration:kDuration animations:^{
        weakSelf.locatePicker.top -= kViewH;
    }];
